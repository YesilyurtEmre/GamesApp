...
extension FavouritesVC: UITableViewDataSource, UITableViewDelegate {
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    guard let cell = tableView.dequeueReusableCell(
      withIdentifier: FavouritesCell.identifier,
      for: indexPath) as? FavouritesCell else {
      fatalError("FavouritesCell couldn't be dequeued")
    }
    cell.configure(with: viewModel.favouriteGame(at: indexPath.row))
    return cell
  }
  func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    let selectedGame = viewModel.favouriteGame(at: indexPath.row)
    let detailVC = GamesDetailVC()
    detailVC.isFromFavourites = true
    detailVC.gameItem = viewModel.convertToGameViewModelItem(from: selectedGame)
    detailVC.gameId = Int(selectedGame.id)
