...
class FavouritesViewModel {
  ...
  func convertToGameViewModelItem(from favouriteGame: FavouriteGame) -> GameViewModelItem {
    return GameViewModelItem(
      id: Int(favouriteGame.id),
      title: favouriteGame.name ?? "",
      metacritic: favouriteGame.metacritic ?? "",
      genres: favouriteGame.genres ?? "",
      imageURL: favouriteGame.imageURL ?? ""
    )
  }
}
